(ns agones-compat.main
  "Kotodama WASM entrypoint for the Agones clean-room actor (L4) — Clojure port.
  ...
  facts (`agones.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [kotoba.lang.text :as str]))

(def ns-prefix "agones")
(def tier "L4")
...
(defn page-limit
  ...
  src/agones_compat/page_limit.kotoba."
  [requested]
  (if (pos? requested)
...
(defn as-int-kernel
  ...
  of as-int stays here as the retained oracle."
  [v]
  (if (pos? v) v 0))
...
(defn emit-facts
  "EAVT facts for one record: {\"agones.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "agones-compat" :tier tier :entities entities} 200])
...
(defn start! [] :agones-compat/ready)
